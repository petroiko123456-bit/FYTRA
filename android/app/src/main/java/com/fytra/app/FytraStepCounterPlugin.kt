...
class FytraStepCounterPlugin : Plugin(), SensorEventListener {
    ...
    fun getDebugLogs(call: PluginCall) {
        try {
            val process = Runtime.getRuntime().exec(arrayOf("logcat", "-d", "-t", "400"))
            val allLines = process.inputStream.bufferedReader().readText()
            val relevant = allLines.lines().filter {
                it.contains("FytraDebug") || it.contains("AndroidRuntime") || it.contains("System.err") || it.contains("Capacitor")
            }.joinToString("\n")
            val ret = JSObject()
