...
class FytraStepCounterPlugin : Plugin(), SensorEventListener {
    ...
    private val MIN_STEP_INTERVAL_MS = 280L       // ~214 βήματα/λεπτό, ταχύτατο περπάτημα/τρέξιμο - όριο
    private val MAX_STEP_INTERVAL_MS = 900L       // ~66 βήματα/λεπτό, πολύ αργό περπάτημα - όριο
    private val REQUIRED_CONSECUTIVE_FOR_WALKING = 3 // πόσα συνεχόμενα ρυθμικά "χτυπήματα" χρειάζονται πριν πιστέψουμε ότι είναι πραγματικό περπάτημα
    private var pendingPeakTimestamps = mutableListOf<Long>()
    ...
    private fun resetAlgorithmState() {
        ...
        algoEventCount = 0L
        pendingPeakTimestamps = mutableListOf()
    ...
    private fun processAccelSample(event: SensorEvent) {
        ...
        if (hp > threshold && !rising && (now - lastPeakTimeMs) > MIN_STEP_INTERVAL_MS) {
            ...
            lastPeakTimeMs = now
            handleCandidatePeak(now, interval)
        } else if (hp < -threshold * 0.3) {
            rising = false
        }
    }

    // Αποφασίζει αν ένα νέο "χτύπημα" (πιθανό βήμα) πρέπει τελικά να μετρηθεί, με βάση το αν εντάσσεται
    // σε μια συνεχόμενη, ρυθμική ακολουθία παρόμοιων χρονικών διαστημάτων (= περπάτημα), ή αν είναι
    // μεμονωμένο/ακανόνιστο (= τυχαία κίνηση χεριού, σήκωμα κινητού, κ.λπ. — αγνοείται εντελώς).
    private fun handleCandidatePeak(now: Long, intervalFromPrevious: Long) {
        val intervalIsWalkingLike = intervalFromPrevious in MIN_STEP_INTERVAL_MS..MAX_STEP_INTERVAL_MS

        if (isWalkingConfirmed) {
            if (intervalIsWalkingLike) {
                creditOneStep(now)
            } else {
                // Ο ρυθμός "έσπασε" (π.χ. σταμάτησες να περπατάς) — ξαναρχίζουμε από την αρχή τον έλεγχο
                // ρυθμικότητας πριν ξαναμετρήσουμε οτιδήποτε άλλο.
                isWalkingConfirmed = false
                pendingPeakTimestamps = mutableListOf(now)
            }
            return
        }

        if (intervalFromPrevious == -1L || !intervalIsWalkingLike) {
            // Πρώτο χτύπημα, ή διάστημα εκτός λογικού εύρους περπατήματος -> ξεκινάμε νέο "υποψήφιο" ρυθμό.
            pendingPeakTimestamps = mutableListOf(now)
            return
        }

        pendingPeakTimestamps.add(now)
        if (pendingPeakTimestamps.size >= REQUIRED_CONSECUTIVE_FOR_WALKING) {
            // Βρέθηκε αρκετά συνεχόμενος ρυθμός -> επιβεβαιώνουμε ότι είναι πραγματικό περπάτημα, και
            // πιστώνουμε ΚΑΙ τα ήδη-συγκεντρωμένα "υποψήφια" χτυπήματα ως βήματα (όχι μόνο το τελευταίο),
            // ώστε να μη χάνονται τα πρώτα πραγματικά βήματα ενός περπατήματος όσο περιμέναμε επιβεβαίωση.
            isWalkingConfirmed = true
            val toCredit = pendingPeakTimestamps.toList()
            pendingPeakTimestamps = mutableListOf()
            toCredit.forEach { creditOneStep(it) }
        }
    }
