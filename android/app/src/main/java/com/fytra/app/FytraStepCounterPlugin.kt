...
class FytraStepCounterPlugin : Plugin(), SensorEventListener {
    ...
    companion object {
        ...
        // ΔΙΟΡΘΩΣΗ ΚΡΙΣΙΜΟΥ BUG (πολλαπλά, παράλληλα "αντίγραφα" του αλγορίθμου μέτρησης): επειδή το
        // Foreground Service κρατάει τη διεργασία ζωντανή ΑΚΟΜΑ ΚΙ ΟΤΑΝ η Activity/WebView καταστρέφεται
        // και ξαναδημιουργείται (κάθε νέο άνοιγμα της εφαρμογής), κάθε τέτοιο άνοιγμα δημιουργούσε ΝΕΟ
        // αντικείμενο αυτού του plugin -> ΝΕΕΣ registerListener() κλήσεις -> ΧΩΡΙΣ ΠΟΤΕ να απεγγράφεται
        // το ΠΡΟΗΓΟΥΜΕΝΟ, "ορφανό" αντικείμενο (αφού κανείς δεν καλεί ποτέ stop() ανάμεσα σε ανοίγματα).
        // Αποτέλεσμα: πολλαπλά, ανεξάρτητα, παράλληλα αντίγραφα του God Mode/v1/v2 filter έτρεχαν
        // ΤΑΥΤΟΧΡΟΝΑ, επεξεργαζόμενα τα ΙΔΙΑ δεδομένα αισθητήρα το καθένα ξεχωριστά — πιθανή αιτία
        // ασυνεπούς/διπλής πίστωσης βημάτων. Αυτή η στατική αναφορά κρατάει ΠΟΙΟ είναι το τρέχον, μοναδικό
        // "ενεργό" αντίγραφο σε όλη τη διεργασία· το start() (βλ. παρακάτω) την χρησιμοποιεί για να
        // απεγγράψει ΠΛΗΡΩΣ οποιοδήποτε προηγούμενο αντίγραφο πριν ξεκινήσει το δικό του.
        @Volatile private var activeInstance: FytraStepCounterPlugin? = null
        ...
        // ΝΕΟ: ΔΕΥΤΕΡΟΣ, ΑΝΕΞΑΡΤΗΤΟΣ μηχανισμός ασφαλείας — καλύπτει μια εντελώς διαφορετική περίπτωση
        // απώλειας βημάτων από το checkClosedAppSteps() παραπάνω (εκείνο βασίζεται στον hardware Android
        // step counter· αυτό εδώ βασίζεται στο ΔΙΚΟ ΜΑΣ God Mode/v1). Επειδή το Foreground Service κρατάει
        // τη διεργασία (και άρα το God Mode) ζωντανή ΑΚΟΜΑ ΚΙ ΑΦΟΥ καταστραφεί το JS/WebView (π.χ. μετά από
        // "κλείσιμο όλων των καρτελών"), το God Mode μπορεί να πιστώσει ένα πραγματικό βήμα ΤΗ ΣΤΙΓΜΗ που
        // δεν υπάρχει ΚΑΝΕΝΑΣ ζωντανός JS listener να το ακούσει — το notifyListeners() πάει στο κενό και
        // το βήμα χάνεται ΜΟΝΙΜΑ, χωρίς κανένα σφάλμα. Αυτό το κλειδί μετράει ΚΑΘΕ πίστωση βήματος σε
        // μόνιμη μνήμη· το JS "επιβεβαιώνει" την παραλαβή κάθε ζωντανού event (μηδενίζοντας το μετρητή) —
        // αν κάτι πιστωθεί ενώ δεν υπάρχει ζωντανό JS να επιβεβαιώσει, ο μετρητής ΣΥΣΣΩΡΕΥΕΤΑΙ, και το JS
        // το ανακτά ολόκληρο στο επόμενο άνοιγμα.
        const val KEY_PENDING_UNACKED_STEPS = "pendingUnackedSteps"
    ...
    fun start(call: PluginCall) {
        ...
        try {
            ...
            // ΔΙΟΡΘΩΣΗ ΚΡΙΣΙΜΟΥ BUG: απεγγράφουμε ΠΛΗΡΩΣ οποιοδήποτε προηγούμενο, "ορφανό" αντίγραφο
            // αυτού του plugin (βλ. πλήρες σχόλιο στο activeInstance στο companion object παραπάνω) ΠΡΙΝ
            // κάνουμε οτιδήποτε άλλο. Το unregisterListener(listener) χωρίς συγκεκριμένο sensor απεγγράφει
            // ΑΥΤΟΜΑΤΑ ΟΛΟΥΣ τους αισθητήρες που είχε εκείνο το αντίγραφο (accel, gravity, gyro, βαρόμετρο,
            // step counter/detector) σε ΜΙΑ κίνηση. Καμία επίδραση στη λογική μέτρησης του ΤΡΕΧΟΝΤΟΣ
            // αντιγράφου — μόνο εξαφανίζει τυχόν παλιά, ξεχασμένα αντίγραφα που έτρεχαν παράλληλα.
            val previousActive = activeInstance
            if (previousActive != null && previousActive !== this) {
                try {
                    previousActive.sensorManager.unregisterListener(previousActive)
                    previousActive.sensorThread?.quitSafely()
                    previousActive.v2AuxSensorThread?.quitSafely()
                    Log.d(TAG, "start: απεγγράφηκε πλήρως ένα προηγούμενο, 'ορφανό' αντίγραφο του plugin")
                } catch (e: Exception) {
                    Log.d(TAG, "start: σφάλμα καθαρισμού προηγούμενου αντιγράφου (μη κρίσιμο): " + e.message)
                }
            }
            activeInstance = this
    ...
    fun checkClosedAppSteps(call: PluginCall) {
        try {
            ...
            // ΔΙΟΡΘΩΣΗ ΚΡΙΣΙΜΟΥ BUG (πάντα delta=0): πριν, η "παλιά" αποθηκευμένη τιμή διαβαζόταν ΜΕΣΑ στο
            // callback, ΑΦΟΥ είχε ήδη φτάσει το νέο sensor event. Επειδή το start() έχει ΞΕΧΩΡΙΣΤΟ, μόνιμο
            // listener στον ΙΔΙΟ αισθητήρα που ενημερώνει την ΙΔΙΑ αποθηκευμένη τιμή σε ΚΑΘΕ event (χωρίς
            // εγγύηση σειράς εκτέλεσης από το Android ανάμεσα στους δύο listeners), αν εκείνο πρόλαβε πρώτο,
            // η "παλιά" τιμή είχε ήδη αντικατασταθεί με τη "νέα" ΠΡΙΝ προλάβουμε να τη διαβάσουμε — άρα η
            // διαφορά έβγαινε ΠΑΝΤΑ 0, ό,τι κι αν είχε γίνει όσο η εφαρμογή ήταν κλειστή. Τώρα διαβάζουμε
            // την παλιά τιμή ΕΔΩ, ΠΡΙΝ εγγραφούμε καθόλου listener — γι' αυτό είναι εγγυημένα "παγωμένη"
            // στην τιμή από πριν, ανεξάρτητα από το τι θα κάνει ο άλλος listener μετά.
            val prefs = context.getSharedPreferences(CLOSED_APP_PREFS_NAME, android.content.Context.MODE_PRIVATE)
            val hasStoredBefore = prefs.contains(KEY_LAST_RAW_STEP_COUNTER)
            val storedValueBefore = prefs.getFloat(KEY_LAST_RAW_STEP_COUNTER, 0f)
            ...
            oneShotListener = object : SensorEventListener {
                override fun onSensorChanged(event: SensorEvent) {
                    ...
                    try {
                        ...
                        if (hasStoredBefore) {
                            if (rawValue >= storedValueBefore) {
                                recoveredDelta = (rawValue - storedValueBefore).toInt()
                            } else {
                                rebootDetected = true
                        ...
                        Log.d(TAG, "checkClosedAppSteps: hasStoredBefore=$hasStoredBefore storedValueBefore=$storedValueBefore rawValue=$rawValue recoveredDelta=$recoveredDelta rebootDetected=$rebootDetected")
                        val ret = JSObject()
    ...
    // ΝΕΟ: ο JS καλεί αυτό ΑΜΕΣΩΣ κάθε φορά που λαμβάνει ΚΑΝΟΝΙΚΑ (ζωντανά) ένα stepCounterUpdate event —
    // "επιβεβαιώνει" ότι το παρέλαβε, ώστε ο μετρητής KEY_PENDING_UNACKED_STEPS να ΜΗΝ συσσωρεύεται όσο η
    // εφαρμογή είναι κανονικά ανοιχτή. Fire-and-forget, καμία επίδραση στο God Mode/v1/v2.
    @PluginMethod
    fun acknowledgeNativeStep(call: PluginCall) {
        try {
            val prefs = context.getSharedPreferences(CLOSED_APP_PREFS_NAME, android.content.Context.MODE_PRIVATE)
            prefs.edit().putInt(KEY_PENDING_UNACKED_STEPS, 0).apply()
            call.resolve()
        } catch (e: Exception) {
            call.reject("acknowledgeNativeStep crashed: " + e.javaClass.simpleName + ": " + e.message)
        }
    }

    // ΝΕΟ: ο JS καλεί αυτό ΜΙΑ φορά, στην αρχή κάθε νέας session (μαζί με το checkClosedAppSteps) — δίνει
    // πίσω ΟΣΑ βήματα πιστώθηκαν από το God Mode ΧΩΡΙΣ να υπάρχει ζωντανός JS listener να τα παραλάβει
    // (π.χ. επειδή η εφαρμογή "έκλεισε" αλλά η διεργασία συνέχισε να τρέχει μέσω του Foreground Service),
    // και μηδενίζει τον μετρητή. Καμία επίδραση στο God Mode/v1/v2 — απλά ανακτά ό,τι χάθηκε λόγω απουσίας
    // JS. Βλ. πλήρες σχόλιο στο KEY_PENDING_UNACKED_STEPS στο companion object.
    @PluginMethod
    fun getAndClearPendingNativeSteps(call: PluginCall) {
        try {
            val prefs = context.getSharedPreferences(CLOSED_APP_PREFS_NAME, android.content.Context.MODE_PRIVATE)
            val pending = prefs.getInt(KEY_PENDING_UNACKED_STEPS, 0)
            prefs.edit().putInt(KEY_PENDING_UNACKED_STEPS, 0).apply()
            Log.d(TAG, "getAndClearPendingNativeSteps: pendingSteps=$pending")
            val ret = JSObject()
            ret.put("pendingSteps", pending)
            call.resolve(ret)
        } catch (e: Exception) {
            call.reject("getAndClearPendingNativeSteps crashed: " + e.javaClass.simpleName + ": " + e.message)
        }
    }
    ...
    private fun creditOneStep(atTimeMs: Long) {
        internalStepCount++
        // ΝΕΟ: μόνιμη καταγραφή ΚΑΘΕ πίστωσης βήματος, ανεξάρτητα από το αν υπάρχει ζωντανός JS listener
        // να το ακούσει αυτή τη στιγμή — βλ. πλήρες σχόλιο στο KEY_PENDING_UNACKED_STEPS στο companion
        // object. Αυτό ΔΕΝ επηρεάζει καθόλου το ίδιο το notifyListeners() παρακάτω ή τον αλγόριθμο.
        try {
            val prefs = context.getSharedPreferences(CLOSED_APP_PREFS_NAME, android.content.Context.MODE_PRIVATE)
            val current = prefs.getInt(KEY_PENDING_UNACKED_STEPS, 0)
            prefs.edit().putInt(KEY_PENDING_UNACKED_STEPS, current + 1).apply()
        } catch (e: Exception) { /* μη κρίσιμο — δεύτερος μηχανισμός ασφαλείας, δεν αγγίζει το God Mode */ }
