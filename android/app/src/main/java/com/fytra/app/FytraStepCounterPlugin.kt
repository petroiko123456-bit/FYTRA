...
class FytraStepCounterPlugin : Plugin(), SensorEventListener {
    ...
    private fun recordGravityDrift() {
        val nowMs = System.currentTimeMillis()
        gravityHistory.addLast(nowMs to gravity.copyOf())
        ...
        val mag1 = sqrt((gravity[0] * gravity[0] + gravity[1] * gravity[1] + gravity[2] * gravity[2]).toDouble())
        if (mag0 < 0.1 || mag1 < 0.1) return
        val dot = (g0[0] * gravity[0] + g0[1] * gravity[1] + g0[2] * gravity[2]) / (mag0 * mag1)
    ...
    private fun processAccelSample(event: SensorEvent) {
        ...
        if (hp > threshold && !rising && (now - lastPeakTimeMs) > MIN_STEP_INTERVAL_MS) {
            ...
            if (hasProperWaveShape) {
                ...
                recordGravityDrift() // ΝΕΟ: μόνο διαγνωστικό, βλ. σχόλιο στη συνάρτηση
    ...
    private fun godModeShouldSuppressCredit(gyroPenalty: Double, amplitude: Double): Boolean {
        ...
        // αξιόπιστο "τελικό λόγο". Έτσι, όποτε γυροσκόπιο Ή ένταση δείχνουν σταθερά ύποπτα, ΤΩΡΑ
        // μπλοκάρεται ΠΑΝΤΑ — το βαρόμετρο παραμένει μόνο διαγνωστικό (καταγράφεται στα logs).
