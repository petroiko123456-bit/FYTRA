...
class FytraStepCounterPlugin : Plugin(), SensorEventListener {
    ...
    override fun load() {
        ...
        try {
            ...
            // ΠΡΟΣΩΡΙΝΟ, ΜΟΝΟ ΓΙΑ ΕΛΕΓΧΟ (καμία χρήση αλλού ακόμα): ελέγχουμε αν η συσκευή διαθέτει
            // βαρόμετρο (TYPE_PRESSURE) — ΔΕΝ το εγγράφουμε, ΔΕΝ το χρησιμοποιούμε πουθενά, μόνο καταγραφή.
            ...
            Log.d(TAG, "load() ΤΕΛΕΙΩΣΕ. stepCounterSensor=" + stepCounterSensor + " stepDetectorSensor=" + stepDetectorSensor +
                ...
                " [BAROMETER CHECK] barometerSensor=" + barometerSensor)
        } catch (e: Exception) {
    ...
    private fun processAccelSample(event: SensorEvent) {
        ...
        // ΚΑΙ το κοινό hasProperWaveShape, ώστε το v2 να έχει την ΙΔΙΑ άμυνα ενάντια σε απότομα artifacts.
        processV2Shadow(now, hp, hasProperWaveShape)
    }
    ...
    private fun processV2Shadow(now: Long, hp: Double, hasProperWaveShape: Boolean) {
        v2PushSample(hp)
