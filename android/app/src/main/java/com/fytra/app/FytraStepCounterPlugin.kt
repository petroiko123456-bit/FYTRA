...
class FytraStepCounterPlugin : Plugin(), SensorEventListener {
    ...
    companion object {
        ...
        const val V2_AUTOCORR_MIN_PERIODICITY = 0.35     // ελάχιστη κανονικοποιημένη συσχέτιση για να θεωρηθεί "ρυθμικό σαν βάδισμα" [0..1]
        ...
        const val V2_GYRO_HIGH_RATIO = 2.6               // πόσες φορές πάνω από τη βάση θεωρείται "έντονη περιστροφή" (πιθανό χειρονομία, όχι βάδισμα)
    ...
    fun start(call: PluginCall) {
        ...
        try {
            ...
            // ΝΕΟ (v2 μόνο, δεν επηρεάζει το v1): gravity/gyroscope στο ΙΔΙΟ background thread.
            osGravityAvailable = false
            osGravitySensor?.let {
                val ok = sensorManager.registerListener(this, it, SensorManager.SENSOR_DELAY_GAME, sensorHandler)
                Log.d(TAG, "start: registerListener (v2 gravity sensor) = " + ok)
            }
            if (hasGyroscope) {
                val ok = sensorManager.registerListener(this, gyroSensor, SensorManager.SENSOR_DELAY_GAME, sensorHandler)
                Log.d(TAG, "start: registerListener (v2 gyroscope) = " + ok)
            }
