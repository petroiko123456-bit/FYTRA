...
class FytraStepCounterPlugin : Plugin(), SensorEventListener {
    ...
    private var trendSlow = 0.0                   // πολύ αργή τάση (DC) του σήματος — αφαιρείται για καθαρό "bandpass" γύρω από το μηδέν
    private var trendInit = false
    private var lastSampleSign = 0                // πρόσημο του τελευταίου δείγματος (για ανίχνευση zero-crossing)
    private var lastRisingZeroCrossMs = 0L         // πότε είδαμε τελευταία φορά το σήμα να περνά από αρνητικό σε θετικό
    private val ZERO_CROSS_TO_PEAK_MAX_MS = 400L   // ένα πραγματικό βήμα έχει peak ΛΙΓΟ μετά από τέτοιο crossing, όχι πολύ αργότερα
    ...
    private val REQUIRED_CONSECUTIVE_FOR_WALKING = 3 // πόσα συνεχόμενα ρυθμικά "χτυπήματα" χρειάζονται πριν πιστέψουμε ότι είναι πραγματικό περπάτημα (μειώθηκε από 5, βλ. σχόλιο πιο κάτω)
    ...
    private fun resetAlgorithmState() {
        ...
        trendInit = false
        lastSampleSign = 0
        lastRisingZeroCrossMs = System.currentTimeMillis() // "prime" ώστε το πρώτο πραγματικό peak να μην απορριφθεί άδικα
        lowPassMag = 0.0
        lowPassInit = false
        ampEstimate = 1.2
        rising = false
    ...
    private fun processAccelSample(event: SensorEvent) {
        ...
        if (usingVerticalProjection) {
            ...
            // 5) ΝΕΟ: αφαιρούμε ΚΑΙ την πολύ αργή "τάση" (DC drift) του ίδιου του κατακόρυφου σήματος —
            // ουσιαστικά ζωνοπερατό (bandpass) φίλτρο γύρω από το μηδέν, standard τεχνική βηματομέτρησης.
            // Χωρίς αυτό, μικρές αργές κλίσεις του χεριού/κινητού μπορούσαν να μετατοπίσουν το σήμα και να
            // μπερδέψουν το προσαρμοστικό όριο.
        ...
        // ΝΕΟ: ανίχνευση zero-crossing — πότε το σήμα περνάει από αρνητικό σε θετικό. Ένα πραγματικό βήμα
        // έχει την κορυφή του ΛΙΓΟ μετά από τέτοιο crossing (ομαλό, φυσικό σχήμα κύματος). Ένα τυχαίο
        // "τίναγμα"/χτύπημα συχνά ΔΕΝ έχει αυτό το καθαρό σχήμα, ακόμα κι αν τυχαία ξεπεράσει το όριο.
        val deadzone = 0.05
